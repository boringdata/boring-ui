...
These are intentionally separate from the PTY WebSocket router so they can be
mounted under the canonical `/api/v1/pty/*` family.
"""
...
def create_pty_lifecycle_router(config: APIConfig | None = None) -> APIRouter:
    ...
    async def create_session() -> dict[str, str]:
        # The PTY websocket accepts a client-provided session_id for creation.
