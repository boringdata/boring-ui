 ...
 *
 * Extracted from App.jsx lines 610-699. Creates review panels for new
 * approvals, closes panels for dismissed ones, and updates params on
 * existing panels.
 */

import { useEffect } from 'react'
import { getReviewTitle } from '../utils/approvalUtils'

/**
 * Syncs review panels with current approvals.
 *
 ...
 * @param {Function} options.normalizeApprovalPath - Path normalization function
...
export function useApprovalPanels({
  ...
  useEffect(() => {
    ...
    // Create or update panels for current approvals
    approvals.forEach((approval) => {
      ...
      if (existingPanel) {
        ...
        existingPanel.api.setTitle(getReviewTitle(approval, projectRoot))
        return
      }
      ...
      const panel = dockApi.addPanel({
        id: panelId,
        component: 'review',
        title: getReviewTitle(approval, projectRoot),
        position,
        params,
      })
      ...
      if (panel?.group) {
        ...
        if (panelMinRef?.current?.center !== undefined) {
