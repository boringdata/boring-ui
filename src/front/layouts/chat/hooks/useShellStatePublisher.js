 ...
 * @param {string}       [params.drawerMode]      - Current browse drawer mode
...
export function useShellStatePublisher({
  ...
  useEffect(() => {
    ...
    const snapshot = createShellStateSnapshot({
      activeDestination,
      drawerOpen,
      drawerMode: drawerMode || 'sessions',
      surfaceCollapsed,
      activeArtifactId,
    ...
    try {
      window.__BORING_UI_CHAT_SHELL_STATE__ = snapshot
      const event = new CustomEvent('boring-ui:shell-state', { detail: snapshot })
      window.dispatchEvent(event)
    } catch {
      // Swallow — dispatching is best-effort
    }
  }, [activeDestination, drawerMode, drawerOpen, surfaceCollapsed, activeArtifactId, orderedIds, activeSessionId])
}
