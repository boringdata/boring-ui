...
import { SURFACE_OPEN_FILE_BRIDGE } from '../../layouts/chat/hooks/useToolBridge'
...
export default function usePanelActions({
  ...
  useEffect(() => {
    // When the chat-centered shell is active, its useToolBridge owns these
    // bridge functions. Skip registration to avoid overwriting them.
    const chatShellOwns = typeof window[SURFACE_OPEN_FILE_BRIDGE] === 'function'
    if (chatShellOwns) return
