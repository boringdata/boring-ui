...
export const THINKING_LEVELS = ['off', 'low', 'high']
...
const MODEL_CANDIDATES = [
  ...
  { provider: 'google', modelId: 'gemini-2.5-flash', label: 'Gemini Flash' },
]

function resolveDefaultApiKey(provider) {
...
export class PiAgentCoreTransport {
  ...
  constructor({ tools, getApiKey, convertToLlm, sessionId } = {}) {
    this._tools = tools || []
    this._getApiKey = getApiKey || resolveDefaultApiKey
    ...
    this._thinkingLevel = 'off' // 'off' | 'low' | 'high'
    this._selectedModel = null // { provider, modelId } or null for auto
  }
  ...
  /**
   * Set thinking level for the next agent turn.
   * @param {'off'|'low'|'high'} level
   */
  setThinkingLevel(level) {
    this._thinkingLevel = THINKING_LEVELS.includes(level) ? level : 'off'
    if (this._agent) {
      this._agent.state.thinkingLevel = this._thinkingLevel
    }
  }

  /** @returns {'off'|'low'|'high'} */
  get thinkingLevel() {
    return this._thinkingLevel
  }

  /**
   * Set the model for the next agent turn. Resets the agent so it
   * reinitializes with the new model on the next sendMessages().
   * @param {string} provider - e.g. 'anthropic', 'openai', 'google'
   * @param {string} modelId - e.g. 'claude-sonnet-4-5-20250929'
   */
  setModel(provider, modelId) {
    this._selectedModel = provider && modelId ? { provider, modelId } : null
    // Force agent recreation on next send
    this.resetAgent({ id: this._sessionId })
  }
  ...
  /**
   * Returns the list of model candidates with availability info.
   * Async because it checks API keys.
   * @returns {Promise<Array<{ provider: string, modelId: string, label: string, available: boolean }>>}
   */
  async getAvailableModels() {
    const getKey = this._getApiKey
    const results = await Promise.all(
      MODEL_CANDIDATES.map(async (c) => {
        const key = getKey ? await getKey(c.provider) : ''
        return { ...c, available: Boolean(key) }
      }),
    )
    return results
  }
  ...
  async _initAgent() {
    ...
    // If a specific model was selected, use it directly
    let model = null
    if (this._selectedModel) {
      model = getModel(this._selectedModel.provider, this._selectedModel.modelId)
    }

    // Otherwise, auto-select first model with an available API key
    if (!model) {
      for (const { provider, modelId } of MODEL_CANDIDATES) {
        const candidate = getModel(provider, modelId)
        if (!candidate) continue

        const key = this._getApiKey ? await this._getApiKey(provider) : ''
        if (key) {
          model = candidate
          break
        }
    ...
    const agentOptions = {
      ...
      getApiKey: this._getApiKey || undefined,
      initialState: {
   ...
   */
  async sendMessages({ messages, abortSignal, trigger: _trigger }) {
