...
| `hosted` | Control plane exposed to browsers; proxies privileged file/git/exec calls to a private sandbox service. | `boring-ui API` (public) + `internal_app` (sandbox data plane). | `WORKSPACE_ROOT` for the sandbox internal API (private). Frontend sees `FILESYSTEM_SOURCE` metadata but never directly hits this workspace. | Public routers limited to approval/capabilities/hosted proxy. Privileged ops go through `create_hosted_sandbox_proxy_router` / `create_hosted_compat_router`. |
...
| --- | --- | --- | --- | --- |
| `BORING_UI_RUN_MODE` | local/hosted | **REQUIRED** | Switches app between direct-mode and control-plane proxy logic (`APIConfig.run_mode`). | Local mode mounts `files`/`git` routers directly; hosted mode relies on `internal_app` plus `hosted_proxy`. |
| `WORKSPACE_ROOT` | all | **REQUIRED** | Sets the workspace path for whichever API instance is running (local app or internal sandbox). | `create_app` defaults to `Path.cwd()`; `internal_app` uses this to drive file/git/exec endpoints. |
...
| `CAPABILITY_PRIVATE_KEY` | hosted | **REQUIRED** (hosted only) | Enables the hosted API to issue capability tokens for the sandbox proxy. | Without this, hosted proxy routes are silently disabled (logged as warning). |
