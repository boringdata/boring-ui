...
def ensure_session(
    ...
    *,
    ...
    if mode == "neon":
        ...
        account_email = email or recipient or f"qa+smoke-neon-{int(time.time())}@mail.boringdata.io"
    ...
    if mode == "supabase":
        ...
        account_email = email or recipient or f"qa+smoke-supabase-{int(time.time())}@mail.boringdata.io"
        account_password = password or random_password()
        if not skip_signup:
            signup_flow(
                client,
